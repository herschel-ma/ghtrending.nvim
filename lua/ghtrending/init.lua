...
function display:init(datas, opts)
    ...
    p:on('BufLeave', function()
      vim.schedule(function()
        ...
        for _, lp in pairs(popups) do
          if lp.bufnr == bufnr then return end
        end
        layout:unmount()
      end)
    end)
  ...
  -- Mapping
  popups.left_popup:map('n', 'q', function() layout:unmount() end, { silent = true })
  popups.left_popup:map('n', '<esc>', function() layout:unmount() end, { silent = true })
  ...
  popups.right_popup:map(
    'n',
    'H',
    function() vim.api.nvim_set_current_win(popups.left_popup.winid) end,
    { silent = true }
  )
    ...
    tbl = nui_table({
      ...
      columns = {
        ...
        },
        {
          ...
          header = 'Star',
          cell = function(cell) return nui_text(tostring(cell.get_value()), 'DiagnosticInfo') end,
        },
        {
          align = 'left',
          accessor_key = 'add',
...
M.setup = function(opts)
  ...
  vim.api.nvim_create_user_command('GhtrendingRepo', function()
    ...
    if M.repos ~= nil then
      ...
      vim.notify('GitHub Api Repositories are nil! Run `:checkhealth ghtrending` for more info.', vim.log.levels.ERROR)
    end
  end, { bang = true })
end
